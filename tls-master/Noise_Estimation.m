function dst_scalars_noise_level=Noise_Estimation(L,shear_f,lpfilt,sigma_n)
...
% Output:
%
% dst_scalars_noise_level            - the cell array containing the scalars of 
...
noise=sigma_n.*randn(L,L);
level=length(shear_f);

% LP decomposition
y_noise = atrousdec(noise,lpfilt,level);

dst_scalars_noise_level=cell(1,level+1);
dst_noise=y_noise{1}; 
dst_scalars_noise_level{1}=median(abs(dst_noise(:) - median(dst_noise(:))))/.6745;
...
for i=1:level, 
    ...
    for k=1:l,          
        ...
        dst_scalars_noise_level{i+1}(k)=median(abs(dst_noise(:) - median(dst_noise(:))))/.6745
    end
end
dst_scalars_noise_level