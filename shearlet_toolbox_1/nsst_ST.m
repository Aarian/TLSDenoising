function dstn=nsst_ST(dst,sigma,Tscalars,dst_scalars)
...
dstn{1} = sign(dst{1}).* max(0,abs(dst{1}) - Tscalars(1)*sigma*dst_scalars{1});
for i=1:level-1,
      l=size(dst{i+1},3);
      for k=1:l, 
           %dstn{i+1}(:,:,k)=dst{i+1}(:,:,k).*(abs(dst{i+1}(:,:,k)) > Tscalars(2)*sigma*dst_scalars{i+1}(k)); 
           dstn{i+1}(:,:,k)=sign(dst{i+1}(:,:,k)).*max(0,abs(dst{i+1}(:,:,k)) - Tscalars(2)*sigma*dst_scalars{i+1}(k)); 
      end
end
...
l=size(dst{i+1},3);
for k=1:l,
      dstn{i+1}(:,:,k)=sign(dst{i+1}(:,:,k)).*max(0,abs(dst{i+1}(:,:,k)) - Tscalars(3)*sigma*dst_scalars{i+1}(k)); 
end