...
for i=1:level-1,
      l=size(dst{i+1},3);
      for k=1:l, 
           dst_denoised = dst{i+1}(:,:,k).*(abs(dst{i+1}(:,:,k)) > Tscalars(2)*sigma*dst_scalars{i+1}(k)); 
           %dst_denoised(isnan(dst_denoised))=0;
           dstn{i+1}(:,:,k)= dst_denoised;
      end
end
...
l=size(dst{i+1},3);
for k=1:l,
      temp  = dst{i+1}(:,:,k).*(abs(dst{i+1}(:,:,k)) > Tscalars(3)*sigma*dst_scalars{i+1}(k)); 
      %temp(isnan(temp)) = 0 ;
      dstn{i+1}(:,:,k)= temp ; 
